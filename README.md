...
You will be prompted only for the items below (vCenter credentials are asked
**only** when the CSV actually contains vCenter VMs):

| Prompt | Description |
|---|---|
| vCenter username | e.g. `administrator@vsphere.local` (only if vCenter VMs present) |
| vCenter password | Hidden input (only if vCenter VMs present) |
| Snapshot description | Default: `Automated snapshot before change` |

**Not prompted (auto-set):**

| Item | Value |
|---|---|
| Snapshot name | Auto-generated as `yyyymmdd-security-Updates` (e.g. `20260624-security-Updates`) |
| VM inventory CSV | Auto-picked: `vm_inventory.csv` in the playbook directory |

The snapshot name uses the run date, so each Azure snapshot is created as
`yyyymmdd-security-Updates-<vm_name>` — a valid Azure resource name (letters,
numbers and hyphens only). To use a different CSV without editing the playbook,
override at runtime: `-e "csv_file=/path/to/other.csv"`.
